library(tidyr)
library(dplyr)
#Categprize all individual members according to their age, skill, principal econoimc activity, and marital status
...
individual_member_details%>%
  group_by(hh_id)%>%
  crosstab(x = activity_group, 
           y = skill_group, 
           weight = Multiplier/100,format = "long")
...
head(merge(head(add_occugrp_col(individual_member_details)),add_skill_col(individual_member_details),by.x="hh_id",by.y="hh_id"))
